...
#Biztinct
class OpenMapsUrlAction(models.Model):
    _name = 'ir.actions.act_window'
    _inherit = 'ir.actions.act_window'

    @api.model
    def read(self, fields=None, load='_classic_read'):
        res = super(OpenMapsUrlAction, self).read(fields=fields, load=load)
        for data in res:
            if data.get('xml_id') == 'fieldservice.action_open_maps_url':  # Replace with your action's XML ID
                context = self.env.context
                maps_url = context.get('maps_url')
                if maps_url:
                    data['url'] = maps_url  # Set the URL from the context
        return res
...
class FSMOrder(models.Model):
    ...
    def _compute_maps_url(self):
        for record in self:
            if record.location_id.partner_latitude and record.location_id.partner_longitude:
                record.maps_url = 'https://www.google.com/maps/dir/?api=1&destination=%s,%s' % (record.location_id.partner_latitude, record.location_id.partner_longitude)
            else:
                record.maps_url = False

    maps_url = fields.Char(compute='_compute_maps_url', string="Map Location")
    ...
    #Biztinct for web_timeline views
    personpartner_id = fields.Integer(string="Person partner Id", compute="_compute_person_partnerid")
    location_name = fields.Char(string="Location name", compute="_compute_locationname")

    @api.depends('person_id')
    def _compute_person_partnerid(self):
        for record in self:
            record.personpartner_id = record.person_id.partner_id.id

    @api.depends('location_id')
    def _compute_locationname(self):
        for record in self:
