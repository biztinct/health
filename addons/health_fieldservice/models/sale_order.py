...
class SaleOrder(models.Model):
    ...
    def _prepare_invoice(self):
        """Override to add fieldservice_order_id to invoice when creating from healthcare quote"""
        invoice_vals = super(SaleOrder, self)._prepare_invoice()

        # Check if this quote is linked to an FSO (via origin or context)
        fso_id = False
        ...
        # Method 2: Get FSO from origin field (format: "FSO00123")
            ...
            fso_ref = self.origin.split(',')[0].strip() if ',' in self.origin else self.origin.strip()
