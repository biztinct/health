...
class HealthStaffAvailabilityMatrix(models.Model):
    ...
    def _compute_location_type(self):
        """Determine location type from associated FSO"""
        for record in self:
            if record.fso_id:
                record.location_type = record.fso_id.service_location
            else:
    ...
    @api.model
    def book_staff_slot(self, staff_id, appointment_datetime, duration_minutes, fso_id=None, assignment_id=None):
        """
        ...
        if fso_id:
            ...
            if fso.exists():
                # FSO model doesn't have buffer times, use defaults
                slot_vals.update({
                    'buffer_before': 15,  # Default 15 minutes
                    'buffer_after': 15    # Default 15 minutes
                })
        ...
        if main_slot.buffer_before > 0:
            ...
            if buffer_start >= 0:  # Don't go before midnight
                buffer_vals = {
                    ...
                    'status': 'buffer',
                    'fso_id': main_slot.fso_id.id if main_slot.fso_id else None,
