...
export class ChordChart extends Component {
  static template = "synconics_bi_dashboard.ChordChart";
  static props = {
    chartId: String,
    name: String,
  ...
  render_chord_chart() {
    ...
    this.root = am5.Root.new("chord_chart__" + this.props.chartId);
    ...
    // Simple flow data: each category flows to "Total"
    const chordData = [];
    ...
    series.nodes.rectangles.template.events.on("click", function (ev) {
      if (self.props.update_chart) {
        self.props.update_chart(
          ...
          "chord_chart",
          { category: ev.target.dataItem.dataContext.id }
        );
      }
    });
