...
class HealthResUserGroupIdsField extends Component {
    static template = "web.ResUserGroupIdsField";
    static components = { Record, FormRenderer };
    static props = { ...standardFieldProps };

    isCheckboxPrivilege(privilege) {
        return (
            privilege &&
            typeof privilege.name === "string" &&
            privilege.name.startsWith("Healthcare")
        );
    }
    ...
    setup() {
        ...
        for (const category of categories) {
            ...
            for (const privilegeId of category.privilege_ids) {
                ...
                if (this.isCheckboxPrivilege(privilege)) {
                    privilege.checkboxFieldNames = [];
                    for (const gid of privilege.group_ids) {
                        const fieldName = this.getGroupFieldName(privilege, gid);
                        privilege.checkboxFieldNames.push(fieldName);
                        this._fields[fieldName] = {
                            help: groups[gid].comment || "",
        ...
        onWillRender(() => {
            ...
            for (const [fieldName, groupId] of Object.entries(this.info.booleanFieldToGroupId)) {
                this.values[fieldName] = selectedIds.has(groupId);
            }
        });
    ...
    getGroupFieldName(privilege, groupId) {
        return `field_${privilege.id}_group_${groupId}`;
    }

    getPrivilegeArch(privilege) {
        if (privilege.checkboxFieldNames && privilege.checkboxFieldNames.length) {
            return privilege.checkboxFieldNames
...
const resUserGroupIdsField = {
    component: HealthResUserGroupIdsField,
    fieldDependencies: [{ name: "view_group_hierarchy", type: "json", readonly: true }],
    additionalClasses: ["w-100"],
};

registry.category("fields").add("res_user_group_ids", resUserGroupIdsField, { force: true });
