...
class HealthAIAssignmentEngine(models.Model):
    ...
    def train_assignment_model(self):
        ...
        try:
            ...
            for assignment in assignments:
                ...
                features = {
                    'appointment_type': self._encode_appointment_type(assignment.appointment_id.appointment_type_id.id),
                    ...
                    'patient_age': assignment.appointment_id.patient_id.age or 30,
            ...
            # Features and target
            feature_columns = ['appointment_type', 'urgency_level', 'assignment_type', 'staff_count', 
    ...
    def _calculate_assignment_success(self, assignment):
        ...
        if assignment.appointment_id.state == 'completed':
            success_score += 0.2
        
        # Patient satisfaction factor (if available)
        if assignment.appointment_id.rating:
            rating_bonus = (float(assignment.appointment_id.rating) - 3) * 0.05  # Scale 1-5 to bonus
    ...
    def _encode_appointment_type(self, type_id):
        """Encode appointment type for ML model"""
        return type_id % 10 if type_id else 0
    ...
    def generate_optimal_assignments(self, target_date=None):
        ...
        # Get unassigned appointments for the target date
        unassigned_appointments = self.env['health.appointment'].search([
        ...
        if not unassigned_appointments:
            return {'status': 'no_unassigned', 'message': 'No unassigned appointments found'}
        
        optimal_assignments = []
        
        for appointment in unassigned_appointments:
            # Get optimal staff suggestions for this appointment
            suggestions = self._get_optimal_staff_suggestions(appointment)
        ...
        return {
            ...
            'total_appointments': len(unassigned_appointments),
            'optimal_assignments': optimal_assignments
        }

    def _get_optimal_staff_suggestions(self, appointment):
        """
        Get optimal staff suggestions for a specific appointment
        """
        # Get available staff for the appointment time
        available_staff = self._get_available_staff(appointment)
        ...
        for staff in available_staff:
            ...
            confidence = self._calculate_assignment_confidence(appointment, staff)
            
            # Get staff skills match
            skills_match = self._calculate_skills_match(appointment, staff)
            
            # Calculate travel efficiency
            travel_efficiency = self._calculate_travel_efficiency(appointment, staff)
            ...
            suggestions.append({
                ...
                'current_workload': self._get_staff_current_workload(staff, appointment.appointment_date),
    ...
    def _get_available_staff(self, appointment):
        """
        Get staff available for the appointment time
        """
