...
| **CC-D** ✅ | Zalo/ZNS refactor: framework adapter w/ correct OAuth v4 + PKCE + rotation locking, NEW fail-closed webhook route (old `/zalo/webhook` becomes 410 shim), per-OA secret verify, zalo.config facade + legacy migration, ZNS readiness card (template approval surfaced separately from chat readiness), Z1–Z9 repairs | **SHIPPED 2026-07-26.** Zalo self-service connect; ZNS honesty. Deviations: the health_zalo→channels dependency edge closes a loop (ledger §5.71) so the coupling is SOFT both ways, `zalo.config` gains no `connection_id` column (joined on the framework's own `(channel, company)` key), and the migration is triggered from the channels module. Blocked on operator checklist §12.5 (a real Zalo app) before anything can connect. |
...
| **CC-G** ✅ | Platform go-live console: `platform_ready()` + `required_platform_keys` replace the count-the-rows availability gate (a half-filled app row no longer lights a card), a "Go live" tab on `channel.platform.app` (redirect URI + webhook URLs this deployment really answers on, per-requirement checklist, the §12 steps that stay human paperwork), a Meta webhook-verify-token generator, and a preflight that runs Meta's documented app-access-token check and answers `unverifiable` — in words — for the three providers that document none. T156–T169 | **SHIPPED 2026-07-29.** Operator-only (`base.group_system`); with zero platform-app rows the tenant Center is byte-identical to CC-F. Deviation: Gmail/Outlook show Odoo core's own redirect URI (`/google_gmail/confirm`, `/microsoft_outlook/confirm`) — printing ours would be a paste the provider refuses. |
